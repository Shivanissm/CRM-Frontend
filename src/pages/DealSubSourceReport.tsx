...
// Shared organization color palette (matches PreSalesDashboard + DealSourceReport)
const chartPalette = ['#4FB6B2', '#E3B04B', '#5DA9E9', '#8E6BBF', '#4CAF8A', '#6A7FDB', '#F28C7D'];
...
export default function DealSubSourceReport() {
  ...
  const subSourceMenuRef = useRef<HTMLDivElement | null>(null);
  const [subSourceSearch, setSubSourceSearch] = useState('');
  ...
  useEffect(() => {
    ...
    const subSourceParam = searchParams.get('subSource');
    ...
    if (subSourceParam) {
      const subSources = subSourceParam.split(',').map(s => s.trim()).filter(s => s);
      setSelectedSubSource(subSources);
    }
    if (statusParam) {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        console.error('Failed to load deal sub-source report data', err);
  ...
  const filteredDeals = useMemo(() => {
    return deals.filter((deal) => {
      ...
      const subSourceMatch =
        selectedSubSource.length === 0 || selectedSubSource.includes(deal.subSource || 'Not captured');

      // Status filter
      const statusMatch = selectedStatus.length === 0 || selectedStatus.includes(deal.status);

      return inRange && pipelineAllowed && orgFilterOk && subSourceMatch && statusMatch;
    });
  }, [accessiblePipelineIds, isPresalesUser, deals, dateRange, selectedOrgIds, selectedSubSource, selectedStatus, accessibleOrgSet]);

  // Build chart data - always show all 4 sub-sources
  const dealSubSourceCategories: BarCategory[] = useMemo(() => {
    const subSources = ['Instagram', 'Whatsapp', 'Landing Page', 'Email'];
    return subSources.map((subSource) => ({ key: subSource, label: subSource }));
  }, []);
  ...
  const simpleBarData = useMemo(() => {
    ...
    // If specific sub-sources are selected, only show those sub-sources
    const categoriesToShow = selectedSubSource.length > 0
      ? dealSubSourceCategories.filter(cat => selectedSubSource.includes(cat.key))
      : dealSubSourceCategories;
    
    categoriesToShow.forEach((cat) => {
      counts[cat.key] = filteredDeals.filter((deal) => {
        const subSourceMatch = (deal.subSource || 'Not captured') === cat.key;
        return subSourceMatch;
      }).length;
    });
    return counts;
  }, [dealSubSourceCategories, filteredDeals, selectedSubSource]);
    ...
    return orgEntries.map((entry) => {
      ...
      dealSubSourceCategories.forEach((cat) => {
        values[cat.key] = filteredDeals.filter((deal) => {
          ...
          const subSourceMatch = (deal.subSource || 'Not captured') === cat.key;
          return orgMatch && subSourceMatch;
        }).length;
      });
      return {
        key: String(entry.id),
        label: entry.name,
        color: colorByOrg.get(entry.id) || chartPalette[0],
        values,
      };
    });
  }, [orgEntries, dealSubSourceCategories, filteredDeals, colorByOrg]);

  const seriesByOrgId = useMemo(() => {
    const map = new Map<number, Series>();
    chartSeries.forEach((series) => {
      const orgId = Number(series.key);
      if (!Number.isNaN(orgId)) {
        map.set(orgId, series);
      }
    });
    return map;
  }, [chartSeries]);
  ...
  const pieSlices: PieSlice[] = useMemo(() => {
    ...
    filteredDeals.forEach((deal) => {
      ...
      if (!orgId) return;
      if (selectedOrgIds.length > 0 && !selectedOrgIds.includes(orgId)) return;
    ...
    return Array.from(counts.entries()).map(([orgId, value]) => {
      ...
      const color = colorByOrg.get(orgId) || chartPalette[0];
      return { key: String(orgId), label, value, color };
    });
  }, [filteredDeals, organizations, selectedOrgIds, colorByOrg]);

  const totals = useMemo(
    () =>
      dealSubSourceCategories.map((cat) => simpleBarData[cat.key] || 0),
    [dealSubSourceCategories, simpleBarData],
  );
  ...
  const toggleSubSourceSelection = (subSource: string) => {
    setSelectedSubSource((prev) => (prev.includes(subSource) ? prev.filter((v) => v !== subSource) : [...prev, subSource]));
  };
  ...
  useEffect(() => {
    const handleClickAway = (e: MouseEvent) => {
      ...
      if (subSourceMenuRef.current && !subSourceMenuRef.current.contains(target)) {
  ...
  const subSourceOptions = ['Instagram', 'Whatsapp', 'Landing Page', 'Email'];
  const statusOptions = [
  ...
  const filteredSubSources = useMemo(() => {
    if (!subSourceSearch.trim()) return subSourceOptions;
    const q = subSourceSearch.toLowerCase();
    return subSourceOptions.filter((s) => s.toLowerCase().includes(q));
  }, [subSourceSearch]);
  ...
  const subSourceLabel = useMemo(() => {
    if (selectedSubSource.length === 0) return 'All Sub-sources';
    return selectedSubSource.join(', ');
  }, [selectedSubSource]);
  ...
  return (
    <div className="deal-source-report-page">
      <header className="deal-source-report-header">
        <h1 style={{ color: '#15803D' }}>Deal Sub-source Report</h1>
      </header>

      {/* Filters Section */}
      <div className="deal-source-filters">
        <div className="deal-source-filter-row">
          <div className="deal-source-filter-group" ref={subSourceMenuRef}>
            <label>Deal Sub-source:</label>
            <button
              type="button"
              className="deal-source-org-button"
              onClick={() => setSubSourceMenuOpen((v) => !v)}
            >
              {subSourceLabel}
            ...
            {subSourceMenuOpen && (
              <div className="deal-source-org-menu">
                <div className="deal-source-org-menu-header">
                  <span>All Sub-sources</span>
                  {selectedSubSource.length > 0 && (
                    <button type="button" onClick={() => setSelectedSubSource([])}>
                      Clear
                    </button>
                  )}
                </div>
                <div className="deal-source-org-menu-list">
                  <input
                    ...
                    value={subSourceSearch}
                    onChange={(e) => setSubSourceSearch(e.target.value)}
                  />
                  <button
                    type="button"
                    className={`deal-source-org-option${selectedSubSource.length === 0 ? ' selected' : ''}`}
                    onClick={() => setSelectedSubSource([])}
                  >
                    All Sub-sources
                  </button>
                  {filteredSubSources.map((subSource) => (
                    <button
                      key={subSource}
                      type="button"
                      className={`deal-source-org-option${selectedSubSource.includes(subSource) ? ' selected' : ''}`}
                      onClick={() => toggleSubSourceSelection(subSource)}
                    >
                      {subSource}
                    </button>
                  ))}
      ...
      <div className="deal-source-chart-section">
        <div className="deal-source-chart-header">
          <h2>Deal Sub-source Distribution</h2>
            ...
            <div className="deal-source-pie-chart">
              <div className="deal-source-pie-visual">
                {(() => {
                  ...
                  pieSlices.forEach((slice) => {
                    ...
                    const color = selectedSubSource.length > 0 && selectedSubSource.includes(slice.key)
                      ? '#64748b' 
                      : slice.color;
                  ...
                  return (
                    <div 
                      ...
                      onClick={() => {
                        ...
                        if (selectedSubSource.length > 0) {
                          setSelectedSubSource([]);
                        }
                      }}
                      title="Click to show all sub-sources"
                    />
                  );
                })()}
              </div>
              <div className="deal-source-pie-legend">
                {pieSlices.map((slice) => {
                  ...
                  const isSelected = selectedSubSource.includes(slice.key);
                  return (
                    <div 
                      key={slice.key} 
                      ...
                      style={{ 
                        cursor: 'pointer',
                        opacity: selectedSubSource.length > 0 && !isSelected ? 0.4 : 1
                      }}
                      onClick={() => {
                        toggleSubSourceSelection(slice.key);
                      }}
            ...
            <div className="deal-source-horizontal-chart">
              <div className="deal-source-horizontal-bars">
                {(selectedSubSource.length > 0
                  ...
                  const orgIdsForStack =
                    selectedOrgIds.length > 0
                      ? selectedOrgIds
                      : orgEntries.map((o) => o.id);
                  const perOrgValues = orgIdsForStack.map((orgId) => {
                    const series = seriesByOrgId.get(orgId);
                    return series ? series.values[cat.key] || 0 : 0;
                  });
                  const totalForCat = perOrgValues.reduce((sum, v) => sum + v, 0);
                  const width =
                    maxTotal > 0 && totalForCat > 0
                      ...
                      <div className="deal-source-horizontal-bar-container">
                        {shouldShowBar ? (
                          <div 
                            ...
                            onClick={() => {
                              toggleSubSourceSelection(cat.key);
                            }}
                          >
                            {perOrgValues.map((value, idx) => {
                              if (value <= 0 || totalForCat <= 0) return null;
                              const orgId = orgIdsForStack[idx];
                              const color =
                                colorByOrg.get(orgId) || chartPalette[idx % chartPalette.length];
                              const segmentWidth = (value / totalForCat) * 100;
                              return (
                                <div
                                  key={`${cat.key}-${orgId}`}
                                  style={{
                ...
                <div className="deal-source-chart-area">
                            ...
                            <div 
                              ...
                              >
                                {perOrgValues.map((value, idx) => {
                                  ...
                                  const color =
                                    colorByOrg.get(orgId) || chartPalette[idx % chartPalette.length];
                                  const segHeight = (value / totalForCat) * 100;
                                  return (
                                    <div
                                      key={`${cat.key}-${orgId}`}
                                      style={{
                  ...
                  <div className="deal-source-chart-labels">
                    {(selectedSubSource.length > 0
                      ...
                      const isSelected = selectedSubSource.includes(cat.key);
                      return (
                        <div 
                          key={cat.key} 
          ...
          <table className="deal-source-table">
            <thead>
              <tr className="deal-source-table-header-row">
                ...
                <th className="deal-source-table-header">Sub-source</th>
              ...
              {filteredDeals.length === 0 ? (
                ...
                filteredDeals.map((deal) => (
                  <tr key={deal.id} className="deal-source-table-row">
                    ...
                    <td className="deal-source-table-cell">{deal.subSource || 'Not captured'}</td>
