...
export default function DealSourceReport() {
  ...
  useEffect(() => {
    ...
    if (sourceParam) {
      const sources = sourceParam.split(',').map(s => s.trim()).filter(s => s);
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        console.error('Failed to load deal source report data', err);
  ...
  const filteredDeals = useMemo(() => {
    return deals.filter((deal) => {
      ...
      // Source filter
      const sourceMatch =
        selectedSource.length === 0 || selectedSource.includes(deal.source || 'Direct');

      // Status filter
      const statusMatch = selectedStatus.length === 0 || selectedStatus.includes(deal.status);

      return inRange && pipelineAllowed && orgFilterOk && sourceMatch && statusMatch;
    });
  }, [accessiblePipelineIds, isPresalesUser, deals, dateRange, selectedOrgIds, selectedSource, selectedStatus, accessibleOrgSet]);

  // Build chart data
  const dealSourceCategories: BarCategory[] = useMemo(() => {
    const sources = ['Direct', 'Divert', 'Planner', 'Reference'];
    return sources.map((source) => ({ key: source, label: source }));
  }, []);
  ...
  const simpleBarData = useMemo(() => {
    ...
    dealSourceCategories.forEach((cat) => {
      counts[cat.key] = filteredDeals.filter((deal) => {
        const sourceMatch = (deal.source || 'Direct') === cat.key;
        return sourceMatch;
      }).length;
    });
    return counts;
  }, [dealSourceCategories, filteredDeals]);
  ...
  }, [orgEntries, dealSourceCategories, filteredDeals, colorByOrg]);
  ...
  const pieSlices: PieSlice[] = useMemo(() => {
    ...
    return Array.from(counts.entries()).map(([orgId, value]) => {
      ...
      const color = colorByOrg.get(orgId) || '#4FB6B2';
  ...
  const totals = useMemo(
    () =>
      dealSourceCategories.map((cat) => simpleBarData[cat.key] || 0),
    [dealSourceCategories, simpleBarData],
  );
  ...
  const dealSourceOptions = ['Direct', 'Divert', 'Planner', 'Reference'];
  const statusOptions = [
  ...
  const filteredSources = useMemo(() => {
    if (!sourceSearch.trim()) return dealSourceOptions;
    const q = sourceSearch.toLowerCase();
    return dealSourceOptions.filter((s) => s.toLowerCase().includes(q));
  }, [sourceSearch]);
  ...
  const sourceLabel = useMemo(() => {
    if (selectedSource.length === 0) return 'All Deal Sources';
    return selectedSource.join(', ');
  }, [selectedSource]);
  ...
  const toggleSourceSelection = (source: string) => {
    setSelectedSource((prev) => (prev.includes(source) ? prev.filter((v) => v !== source) : [...prev, source]));
  };
  ...
  return (
    <div className="deal-source-report-page">
      <header className="deal-source-report-header">
        <h1 style={{ color: '#15803D' }}>Deal Source Report</h1>
      </header>

      {/* Filters Section */}
      <div className="deal-source-filters">
        <div className="deal-source-filter-row">
          <div className="deal-source-filter-group" ref={sourceMenuRef}>
            ...
            {sourceMenuOpen && (
              <div className="deal-source-org-menu">
                ...
                <div className="deal-source-org-menu-list">
                  ...
                  <button
                    type="button"
                    className={`deal-source-org-option${selectedSource.length === 0 ? ' selected' : ''}`}
                  ...
                  {filteredSources.map((source) => (
                    <button
                      key={source}
                      type="button"
                      className={`deal-source-org-option${selectedSource.includes(source) ? ' selected' : ''}`}
      ...
      <div className="deal-source-chart-section">
        <div className="deal-source-chart-header">
          <h2>Deal Source Distribution</h2>
          {selectedOrgIds.length > 0 && (
            <div className="deal-source-chart-org-legend">
              {selectedOrgIds.map((orgId) => {
                const org = organizations.find((o) => o.id === orgId);
                const color = colorByOrg.get(orgId) || chartPalette[0];
                return (
          ...
          {chartView === 'pie' ? (
            <div className="deal-source-pie-chart">
              <div className="deal-source-pie-visual">
                {(() => {
                  ...
                  pieSlices.forEach((slice) => {
                    ...
                    const color = selectedSource.length > 0 && selectedSource.includes(slice.key)
                      ? '#64748b' 
                      : slice.color;
                  ...
                  return (
                    <div 
                      ...
                      title="Click to show all sources"
                    />
                  );
                })()}
              </div>
              <div className="deal-source-pie-legend">
                {pieSlices.map((slice) => {
                  ...
                  const isSelected = selectedSource.includes(slice.key);
                  return (
                    <div 
                      key={slice.key} 
                      ...
                      style={{ 
                        cursor: 'pointer',
                        opacity: selectedSource.length > 0 && !isSelected ? 0.4 : 1
                      }}
                      onClick={() => {
                        toggleSourceSelection(slice.key);
                      }}
            ...
            <div className="deal-source-horizontal-chart">
              <div className="deal-source-horizontal-bars">
                {(selectedSource.length > 0
                  ? dealSourceCategories.filter((cat) => selectedSource.includes(cat.key))
                ...
                <div className="deal-source-chart-area">
                  ...
                  <div className="deal-source-chart-labels">
                    {(selectedSource.length > 0
                      ...
                      const isSelected = selectedSource.includes(cat.key);
                      return (
                        <div 
                          key={cat.key} 
          ...
          <table className="deal-source-table">
            <thead>
              <tr className="deal-source-table-header-row">
                ...
                <th className="deal-source-table-header">Source</th>
              ...
              {filteredDeals.length === 0 ? (
                ...
                filteredDeals.map((deal) => (
                  <tr key={deal.id} className="deal-source-table-row">
                    ...
                    <td className="deal-source-table-cell">{deal.source || 'Direct'}</td>
